...
class InstanceGenerator(_Iterator):
    ...
    process.
    """
    def __init__(self, batch_size, user_dataset, item_dataset,
        ...
        :param item_dataset: The dataset of feature vectors per item
        :type item_dataset: experiments.util.dataset.ItemDataset
        ...
        self.nr_items = len(self.item_dataset)
        ...
        self.user_item_reward = np.zeros((len(self.user_dataset),
                                          len(self.item_dataset)),
                                         dtype=np.int32)
        for i, u in enumerate(self.user_dataset):
            self.user_item_reward[i, u] = 1
        self.sorted_reward_idx = np.argsort(-self.user_item_reward, axis=1)
        self.sorted_reward_loc = np.sum(self.user_item_reward == 1, axis=1)
    ...
    def __next__(self):
        ...
        # samples
        result = np.vstack(self.item_dataset[final_idx])
    ...
    def _sample_per_item(self, us):
        ...
        # first out_size entries because nr_items >> out_size
        out = -np.ones((us.shape[0], self.out_size), dtype=np.int32)
        n = self.nr_items - self.sorted_reward_loc[us]
